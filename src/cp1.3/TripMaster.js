...
function TripMaster() {
  ...
  useEffect(() => {
    if (user) {
      // Reset data states when page changes
      setDestinations([]);
      setVacations([]);
      setUserList([]);
      setBookings([]);
      setSearchTerm(''); // Reset search term when page changes
  ...
  async function loadData(page = currentPage, isSearch = false) {
    if (!user) return;
    ...
    try {
      console.log(`Loading all ${collectionName} data for user ${user.uid}`);
      
      // Get all data for this user (no search term filtering in the query)
      const q = query(
      ...
      // If search is active, filter the items client-side
      ...
      if (isSearch && searchTerm.trim() !== '') {
        ...
        switch (page) {
          case PAGES.DESTINATIONS:
            filteredItems = allItems.filter(item => 
              (item.city && item.city.toLowerCase().includes(searchTermLower)) || 
              (item.country && item.country.toLowerCase().includes(searchTermLower))
            );
            break;
          case PAGES.VACATIONS:
            filteredItems = allItems.filter(item => 
          ...
          case PAGES.USERS:
            filteredItems = allItems.filter(item => 
              ...
              (item.phoneNumber && item.phoneNumber.toLowerCase().includes(searchTermLower))
            );
            break;
          case PAGES.BOOKINGS:
            filteredItems = allItems.filter(item => 
              (item.name && item.name.toLowerCase().includes(searchTermLower)) ||
              (item.phoneNumber && item.phoneNumber.toLowerCase().includes(searchTermLower)) ||
              (item.destination && item.destination.toLowerCase().includes(searchTermLower))
            );
            break;
          default:
            break;
        ...
        console.log(`Search found ${filteredItems.length} items matching "${searchTerm}"`);
      }
      
      // Sort the filtered items by createdAt date (most recent first)
      filteredItems.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
      
      // Update the appropriate state based on page
      switch (page) {
        case PAGES.DESTINATIONS:
          setDestinations(filteredItems);
          break;
        case PAGES.VACATIONS:
          setVacations(filteredItems);
          break;
        case PAGES.USERS:
  ...
  function handleQueryResults(querySnapshot, page) {
    const items = [];
    ...
    switch (page) {
      case PAGES.DESTINATIONS:
        console.log(`Setting destinations state with ${items.length} items`);
  ...
  // Update the handleSearch function to trigger search
  ...
  async function handleSubmitForm(e) {
    ...
    try {
      ...
      if (editingId) {
        // Update existing item - preserve userId and createdAt
        const updateData = { ...formData };
        console.log(`Updating ${collectionName} document:`, editingId, updateData);
        ...
        // Ensure required fields are always set when adding new items
        ...
        console.log(`Adding new ${collectionName} document:`, newItem);
        ...
        switch (currentPage) {
          case PAGES.DESTINATIONS:
            setDestinations(prevDestinations => [addedItem, ...prevDestinations]);
            break;
          case PAGES.VACATIONS:
            setVacations(prevVacations => [addedItem, ...prevVacations]);
            break;
          case PAGES.USERS:
            setUserList(prevUsers => [addedItem, ...prevUsers]);
            break;
          case PAGES.BOOKINGS:
            setBookings(prevBookings => [addedItem, ...prevBookings]);
            break;
          default:
            break;
        }
      ...
      console.error(`Error submitting form for ${currentPage}:`, error);
  ...
  function navigateTo(page) {
    ...
    setUserList([]);
    setBookings([]);
    setSearchTerm(''); // Reset search term when navigating
    setShowForm(false);
  ...
  // Update the search bar component for better mobile responsiveness
    ...
    return (
      <form className="search-bar" onSubmit={handleSearch}>
        <input
          ...
          aria-label="Search"
        />
        <button type="submit" className="btn blue" aria-label="Search">
          Search
        </button>
        <button 
          type="button" 
        ...
        <>
          ...
          <div className="welcome-container">
            <div className="welcome-content">
              ...
              <p>Your complete vacation management solution</p>
              
              {/* Feature cards removed for better mobile experience */}
